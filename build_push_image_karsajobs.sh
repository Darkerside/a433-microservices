...
docker images
docker tag karsajobs:latest ghcr.io/darkerside/karsajobs:latest
echo $GITHUB_TOKEN | docker login ghcr.io --username $GITHUB_USERNAME --password-stdin
docker push ghcr.io/darkerside/karsajobs:latest
...
# docker build -t karsajobs:latest   >> Perintah ini digunakan untuk membangun sebuah image Docker dengan nama karsajobs dan tag latest dari konteks direktori saat ini. Flag -t digunakan untuk menentukan nama dan tag untuk image yang dibangun.
...
# docker tag karsajobs:latest ghcr.io/darkerside/karsajobs:latest  >> Perintah ini memberi nama ulang image Docker karsajobs:latest dengan nama yang lebih spesifik, yaitu ghcr.io/darkerside/karsajobs:latest
...
# docker push ghcr.io/darkerside/karsajobs:latest  >> Perintah ini mengunggah image Docker yang telah dibuat dan diberi nama ke registri Docker di GitHub Container Registry. Pastikan nama image dan tag sudah sesuai dengan yang diinginkan.
# 
####